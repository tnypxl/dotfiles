...
```bash
INIT=$(node /Users/arikj/.config/opencode/get-shit-done/bin/gsd-tools.js init plan-phase "$PHASE" --include state,roadmap,requirements,context,research,verification,uat)
```
...
```bash
PHASE_DESC=$(node /Users/arikj/.config/opencode/get-shit-done/bin/gsd-tools.js roadmap get-phase "${PHASE}" | jq -r '.section')
...
STATE_SNAP=$(node /Users/arikj/.config/opencode/get-shit-done/bin/gsd-tools.js state-snapshot)
