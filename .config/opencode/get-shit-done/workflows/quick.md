...
```bash
INIT=$(node /Users/arik/.config/opencode/get-shit-done/bin/gsd-tools.cjs init quick "$DESCRIPTION")
```
...
```bash
node /Users/arik/.config/opencode/get-shit-done/bin/gsd-tools.cjs commit "docs(quick-${next_num}): ${DESCRIPTION}" --files ${file_list}
```
